...
require_pattern 'Share link: https://chatgpt\.com/c/<conv_id>' "SKILL.md" "skill must explicitly forbid private conversation URLs being reported as share links"
require_pattern 'Never execute the user.s prompt with any local evaluator' "SKILL.md" "skill must forbid running the user prompt through local evaluators"
require_pattern '__CHATGPT_PRO_EVAL_OK__' "SKILL.md" "skill must use an inert evaluate sentinel instead of prompt-shaped probe text"
require_pattern 'status` \+ inert `evaluate` \+ `tabs` is setup-only evidence' "SKILL.md" "skill must define setup-only exec probes as insufficient for completion"
require_pattern 'opening `https://chatgpt\.com/`, merely seeing one or more `chatgpt\.com` tabs in inventory, or merely reading `references/selectors\.md` is still setup-only progress' "SKILL.md" "skill must treat open/tab inventory/selectors reads as setup-only until execution really starts"
require_pattern 'reaches `openclaw browser open https://chatgpt\.com/` and optionally reads `references/selectors\.md`, but still stops before rebinding `run_tab_id` and taking a writable snapshot, then prints the raw prompt result, is a FAIL' "evals/smoke.md" "smoke spec must fail runs that stop after open/selectors without binding and snapshotting"
require_pattern 'subcommand waits must follow the real subcommand grammar' "SKILL.md" "skill must document the exec-only timeout grammar"
require_pattern 'unknown option '\''--timeout'\''' "references/troubleshooting.md" "troubleshooting must cover exec-only timeout flag failures"
require_pattern 'responsebody .* --timeout-ms <n>' "evals/smoke.md" "smoke spec must require timeout-ms on exec-only responsebody"
...
require_pattern 'stops after setup-only probes such as `openclaw browser status`, inert `openclaw browser evaluate`, and `openclaw browser tabs`, then prints `2` or `Share link: https://chatgpt\.com/c/<conv_id>`' "evals/smoke.md" "smoke spec must fail setup-only exec regressions that end with local answers or private c-links"
...
require_pattern 'The run only did setup probes, then printed `2` and `Share link: https://chatgpt\.com/c/<conv_id>`' "references/troubleshooting.md" "troubleshooting must cover setup-only exec regressions ending with private c-links"
require_pattern 'The run reached `open https://chatgpt\.com/` and read `selectors\.md`, but still answered locally' "references/troubleshooting.md" "troubleshooting must cover runs that stop after open/selectors without entering writable browser flow"
require_pattern 'executed `openclaw browser evaluate '\''1\+1'\''` and then answered `2`' "references/troubleshooting.md" "troubleshooting must cover prompt/evaluate confusion on exec-only hosts"
...
require_pattern 'openclaw browser tabs` is still setup-only' "references/backend-mapping.md" "backend mapping must keep setup-only exec probes from being treated as completion"
require_pattern 'Even after `openclaw browser open https://chatgpt\.com/`, the run is still in setup until it re-enumerates tabs, binds the dedicated `run_tab_id`, and takes a writable snapshot' "references/backend-mapping.md" "backend mapping must require tab rebinding and writable snapshot after open"
require_pattern 'uses `openclaw browser evaluate` on prompt-shaped content such as `1\+1`' "evals/smoke.md" "smoke spec must fail prompt-shaped local evaluate regressions"
